...
// Modal popups for projects (with ESC/tap anywhere)
...
function revealOnScroll() {
  ...
  reveals.forEach(function (r) {
    ...
    if (t < wh - 50) r.classList.add("active");
  });
}
...
// Dark mode: auto, toggle, persistent
const dmtog = document.getElementById("darkmode-toggle");
function setDark(isDark) { if (isDark) document.body.classList.add("darkmode"); else document.body.classList.remove("darkmode"); }
...
function storePref(val) {
  try { localStorage.setItem("umang_portfolio_theme", val ? "dark" : "light"); } catch (e) {}
}
...
if (dmtog) dmtog.addEventListener("click", function () { toggleDark(); });
